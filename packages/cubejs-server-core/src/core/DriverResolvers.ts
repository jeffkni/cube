...
export const driverDependencies = (dbType: DatabaseType) => {
  console.log("Resolving driver dbType", dbType);
 ...
 */
export const lookupDriverClass = (
  dbType
): Constructor<BaseDriver> & {
  dialectClass?: () => any;
  getDefaultConcurrency?: () => number;
} => {
  console.log("LookupDriverClass here ...");
  // eslint-disable-next-line global-require,import/no-dynamic-require
  const module = require(driverDependencies(
...
export const getDriverMaxPool = async (
  ...
  if (!options) {
    ...
    const queryQueueOptions = await options.queryCacheOptions.queueOptions(
      context.dataSource
    );

    const preAggregationsQueueOptions =
      await options.preAggregationsOptions.queueOptions(context.dataSource);

    return (
      2 *
      (queryQueueOptions.concurrency + preAggregationsQueueOptions.concurrency)
    );
  }
};
